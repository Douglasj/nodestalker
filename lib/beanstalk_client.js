...
var BeanstalkJob = {};
BeanstalkJob.create = function(data, bindata) {
	if(data.length == 2) {
		return {
            id : data[0],
            data : data[1],
...
BeanstalkCommand.prototype.onSuccess = function(callback) {
	this.addListener('command_done', function(data, bindata) {
		callback(data, bindata);
	});
};
...
BeanstalkCommand.prototype.responseHandler = function(data, obj, callback) {
    ...
    // split after the first line and remove the last two chars (\r\n)
    var bindata = data.slice(lines[0].length+2, (data.length > lines[0].length+4)?data.length-2:undefined);

	if(obj.expected != chunks[0]) {
		this.emit('command_done', chunks, bindata);
		return false;
	}
	...
	if(obj.is_yaml) {
		...
		if(chunks.length > 1) {
			...
			if(jobdata) {
				chunks.pop();
				chunks.push(jobdata);
				chunks = BeanstalkJob.create(chunks, bindata, bindata);
			}
		}
